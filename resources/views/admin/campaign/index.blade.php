...
17 10Z"
          ...
          <form action="{{ route('admin.campaign.index') }}"
                method="GET">
            <input class="form-input w-full rounded-lg pl-10 pr-4"
                   type="text"
                   name="q"
      ...
      <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
        <div class="inline-block min-w-full shadow-sm rounded-lg overflow-hidden">
            ...
            <tbody class="bg-gray-200">
              @forelse($campaigns as $campaign)
                <tr class="border bg-white">
                  <td class="px-5 py-2">{{ $campaign->title }}</td>
                  <td class="px-16 py-2">{{ $campaign->category->name }}</td>
                  <td class="px-16 py-2">{{ moneyFormat($campaign->target_donation) }}</td>
                  <td class="px-16 py-2">{{ $campaign->max_date }}</td>
          ...
          @if ($campaigns->hasPages())
            <div class="bg-white p-3">{{ $campaigns->links('vendor.pagination.tailwind') }}</div>
          @endif
        </div>
      </div>
    </div>
