  ...
  <div class="p-6 max-w-sm w-full bg-white shadow-md rounded-md">
    ...
    <form class="mt-4" action="{{ route('password.update') }}" method="POST">
      ...
      <label class="block">
        ...
        <input type="email" name="email" value="{{ $request->email ?? old('email') }}" class="form-input mt-1 block w-full rounded-md focus:border-indigo-600" placeholder="Alamat Email">
        @error('email')
